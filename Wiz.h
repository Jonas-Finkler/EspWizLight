...
struct LightConfig {
    ...
    int temperature = 0;
    int dimming = 0;
    int sceneId = 0; // 0 means no scene, for other scenes see: https://github.com/sbidy/pywizlight/blob/master/pywizlight/scenes.py
};
...
private:
    ...
    WizResult sendCommand(StaticJsonDocument<JSON_SIZE> command, StaticJsonDocument<JSON_SIZE> &response);
