...
int Wiz::discoverLights(Wiz lights[], int maxNumLights){
    // const char* msg = "{\"method\":\"registration\",\"params\":{\"phoneMac\":\"AAAAAAAAAAAA\",\"register\":false,\"phoneIp\":\"1.2.3.4\",\"id\":\"1\"}}";
    StaticJsonDocument<JSON_SIZE> command;
    // command["method"] = "getPilot"; // the getPilot seems to not work when the lights are off
    ...
    while (true){
        ...
        if (responseState != WizResult::SUCCESS) { // did not get any reply todo: differenciate between errors and timeout
...
WizResult Wiz::sendCommand(StaticJsonDocument<JSON_SIZE> command, StaticJsonDocument<JSON_SIZE> &response){
    WiFiUDP udp;
    // Already start listenting for response
    udp.begin(WIZ_PORT);
...
WizResult Wiz::awaitResponse(WiFiUDP &udp, StaticJsonDocument<JSON_SIZE> &response){
    int start = millis();
    while (start + Wiz::TIMEOUT > millis()){
        ...
        if (packetSize) {
            ...
            // otherwise, the json lib will work with references to packetBuffer, which will soon go out of scope
            DeserializationError error = deserializeJson(response, (const char*) packetBuffer);
            if (error) {
                // todo: Error handling
...
WizResult Wiz::pullConfig(){
    ...
    if (sendResult == WizResult::SUCCESS) {
        ...
        // so we just set it to -1 so we will not send back 0 and cause an error the next time
